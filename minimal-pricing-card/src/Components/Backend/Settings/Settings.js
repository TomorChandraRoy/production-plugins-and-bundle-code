...
const Settings = ({ attributes, setAttributes, device }) => {
	...
	return <>
		<InspectorControls>
			...
			<TabPanel className='bPlTabPanel wp-block-bmpc-minimal-pricing-card' activeClass='activeTab' tabs={generalStyleTabs} onSelect={tabController}
			>
				{
					tab => <>
		...
		<BlockControls>
			...
			<AlignmentToolbar value={alignment} onChange={val => setAttributes({ alignment: val })} describedBy={__('Block Name Alignment')} alignmentControls={[
				{ title: __('Block Name in left', 'b-pricing-card'), align: 'left', icon: 'align-left' },
				{ title: __('Block Name in center', 'b-pricing-card'), align: 'center', icon: 'align-center' },
				{ title: __('Block Name in right', 'b-pricing-card'), align: 'right', icon: 'align-right' }
			]} />
			{/* BplBlockPreview  */}
